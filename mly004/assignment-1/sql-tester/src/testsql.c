...
int query_exec(char const* const query_name,
  ...
  */
int run_query(char const* const query_name, char* const query_res, const size_t res_len, char* const query_err, const size_t err_len) {
  ...
  while ((rc = sqlite3_step(res)) == SQLITE_ROW) {
    if (row == 0) {
      ...
      strncat(query_res, "\n", res_len - strlen(query_res) - 1);
      strncat(query_res, (char*)sqlite3_column_text(res, 0), res_len - strlen(query_res) - 1);
    }
    row++;
  }
