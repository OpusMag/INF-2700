...
typedef struct schema_struct {
  ...
  char *fields;         /**< field names (mly004) */    
...
typedef struct tbl_desc_struct {
  ...
  page_p curr_pg; /**< current page being accessed. */
...
int open_db(void) {
  pager_terminate(); /* first clean up for a fresh beg */
  pager_init();
  read_tbl_descs();
  return 1;
}
...
void set_tbl_position(tbl_p t, tbl_position pos) {
  ...
  case TBL_BEG:
    {
      t->curr_pg = get_page(t->sch->name, 0);
      page_set_pos_begin(t-> curr_pg);
    }
    break;
  case TBL_END:
    t->curr_pg = get_page_for_append(t->sch->name);
  }
}

int eot(tbl_p t) {
  return (peof(t->curr_pg));
}
...
//and set the current page to the result of calling
//the get_page function with the page number and
...
//mly004: copying above code and adjusting for other operations
...
//mly004: copying above code and adjusting for other operations
static int int_lesser_or_equal(int x, int y) {
  return x <= y;
}
//mly004: copying above code and adjusting for other operations
static int int_greater_or_equal(int x, int y) {
  return x >= y;
}
//mly004: copying above code and adjusting for other operations
static int int_lesser(int x, int y) {
  return x < y;
}
...
tbl_p table_search(tbl_p t, char const* attr, char const* op, int val) {
  ...
  if (strcmp(op, "<=") == 0)
    cmp_op = int_lesser_or_equal;

  if (strcmp(op, ">") == 0)
    cmp_op = int_greater;
  
  if (strcmp(op, ">=") == 0)
    cmp_op = int_greater_or_equal;
